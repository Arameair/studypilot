...
venv_dir="${STUDYPILOT_TRANSCRIPTION_VENV:-${repo_root}/tools/transcription-worker/.venv}"

"${python_bin}" -c 'import sys; raise SystemExit(0 if (3, 10) <= sys.version_info[:2] <= (3, 12) else "StudyPilot supports Python 3.10 through 3.12 for operational transcription")'
