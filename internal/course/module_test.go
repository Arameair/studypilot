...
func TestNewModulePlan(t *testing.T) {
	...
	plan, err := NewModulePlan(paths, "TCM Practical Help Desk", 3, "Windows Services", fixedDate)
	if err != nil {
		t.Fatalf("NewModulePlan() error = %v", err)
	}
	root := filepath.Join(coursePlan.Root, "Modules", "03 - Windows Services")
	if plan.Root != root || plan.Scope != studyfs.PlanScopeModule || len(plan.Operations) != 10 {
	...
	for _, required := range []string{
		"id: module-tcm-practical-help-desk-03-windows-services",
		"visibility: private",
		"module_number: 3",
	...
	second, err := NewModulePlan(paths, "TCM Practical Help Desk", 3, "Windows Services", fixedDate)
...
func TestModuleExecutionIsIdempotent(t *testing.T) {
	paths := initializedWorkspace(t)
	coursePlan, err := NewCoursePlan(paths, "TCM Practical Help Desk", fixedDate)
	if err != nil {
		t.Fatalf("NewCoursePlan() error = %v", err)
	...
	plan, err := NewModulePlan(paths, "TCM Practical Help Desk", 12, "Windows Services", fixedDate)
	if err != nil {
		t.Fatalf("NewModulePlan() error = %v", err)
	}
	first, err := studyfs.Execute(plan)
	if err != nil || first.CreatedCount() != 10 || first.HasConflicts() {
		t.Fatalf("first Execute() report/error = %#v / %v", first, err)
	}
	second, err := studyfs.Execute(plan)
	if err != nil || second.SkippedCount() != 10 || second.HasConflicts() {
